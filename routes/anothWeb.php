...
function grabFavIcon()
{
    ...
    // $finalUrl  = $baseUrl . $siteURL;
    $baseTwo = "https://t2.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=";
    $anotherText = "https://stackoverflow.com/";
    $finalTwo  = $baseTwo . $anotherText;
    $path = $finalTwo;
    // file_get_contents($path)
    $content  = file_get_contents($path);

    // fopen() and fwrite() and Then Download the Created File 

    // Storage::disk('public')->put('img.png', file_get_contents($path));
    // $path = Storage::path('img.png');
    // ! Content is the File to pass To Cloudinary 
    dd($content);
}
